...
def _bmm_kernel(
    ...
    stride_abatch,
    stride_an,
    stride_am,  # strides of A
    stride_bbatch,
    stride_bm,
    stride_bp,  # strides of B
    stride_cbatch,
    stride_cn,
    stride_cp,  # strides of C
...
def bmm_kernel_impl(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    print("Hello from bmm_kernel_impl")
    ...
    # Output tensor
    C = torch.empty((BATCH, N_SIZE, P_SIZE), dtype=A.dtype, device=A.device)
