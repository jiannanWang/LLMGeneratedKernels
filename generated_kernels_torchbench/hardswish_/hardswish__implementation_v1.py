...
def _hardswish_kernel(
    x_ptr,  # *only* tensor argument (in-place)
...
def hardswish__kernel_impl(tensor: torch.Tensor) -> torch.Tensor:
    print("Hello from hardswish__kernel_impl")
    ...
    if tensor.dtype not in (torch.float16, torch.bfloat16, torch.float32):
        raise TypeError(f"Unsupported dtype {tensor.dtype}. Only fp16 / bf16 / fp32 are allowed.")
