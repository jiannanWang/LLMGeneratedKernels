...
# ----------------------------------------------------------------------------- #
@triton.jit
def _neg_kernel(ptr_in, ptr_out, numel, BLOCK_SIZE: tl.constexpr, DTYPE: tl.constexpr):
    """
    Parameters
    ----------
...
def neg_kernel_impl(x: torch.Tensor) -> torch.Tensor:
    print("Hello from neg_kernel_impl")
    ...
    """
    if not x.is_cuda:
        raise ValueError("`kernel_function` requires the input tensor to live on a CUDA device.")
