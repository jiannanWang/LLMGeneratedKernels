...
def _eq_scalar_strided_kernel(
    ...
    S5,
    S6,
    S7,  # shape per dimension (padded to 8D)
    STR0,
    STR1,
    STR2,
    STR3,
    ...
    STR5,
    STR6,
    STR7,  # strides per dimension (in elements)
    ...
    i7 = (idx % S7).to(tl.int64)
    idx = idx // S7
    i6 = (idx % S6).to(tl.int64)
    idx = idx // S6
    i5 = (idx % S5).to(tl.int64)
    idx = idx // S5
    i4 = (idx % S4).to(tl.int64)
    idx = idx // S4
    i3 = (idx % S3).to(tl.int64)
    idx = idx // S3
    i2 = (idx % S2).to(tl.int64)
    idx = idx // S2
    i1 = (idx % S1).to(tl.int64)
    idx = idx // S1
    i0 = idx.to(tl.int64)
...
def _eq_scalar_complex_strided_kernel(
    xr_ptr,
    xi_ptr,  # pointers to real and imaginary views (float32/64)
    ...
    STR5,
    STR6,
    STR7,  # strides per dimension (elements of real/imag dtype)
...
def eq_kernel_impl(tensor: torch.Tensor, scalar):
    print("Hello from eq_kernel_impl")
    """
    ...
    _eq_scalar_strided_kernel[grid](
        ...
        out,
        float(scalar),
        int(bool(scalar)) if is_bool else int(scalar),
        N_ELEMENTS,
        shape[0],
        shape[1],
        shape[2],
