...
def _addmm_kernel(
    ...
    for k_tile in range(0, num_k_tiles):
        ...
        a_ptrs = mat1_ptr + (offs_m[:, None] * stride_mat1_row + offs_k[None, :] * stride_mat1_col)
        b_ptrs = mat2_ptr + (offs_k[:, None] * stride_mat2_row + offs_n[None, :] * stride_mat2_col)
    ...
    if BIAS_IS_VECTOR:
        ...
        # bias: (M, N)
        bias_ptrs = bias_ptr + (
            offs_m[:, None] * stride_bias_row + offs_n[None, :] * stride_bias_col
        )
        bias_tile = tl.load(
            bias_ptrs,
...
def addmm_kernel_impl(
    ...
    *,
    ...
    print("Hello from addmm_kernel_impl")
