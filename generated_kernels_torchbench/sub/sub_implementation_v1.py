...
def _patched_sub_tensor(a: torch.Tensor, b: torch.Tensor, *, alpha=1):
    is_integral = a.dtype in (torch.int8, torch.int16, torch.int32, torch.int64)
...
def _broadcast_contiguous(x: torch.Tensor, shape: Tuple[int, ...]) -> torch.Tensor:
    """
...
def sub_kernel_impl(
    tensor_a: torch.Tensor, tensor_b: torch.Tensor, *, alpha: float = 1.0
) -> torch.Tensor:
    ...
    is_int_dtype = tensor_a.dtype in (torch.int8, torch.int16, torch.int32, torch.int64)
