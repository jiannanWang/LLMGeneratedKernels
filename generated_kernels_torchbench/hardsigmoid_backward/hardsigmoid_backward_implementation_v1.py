...
def hardsigmoid_backward_kernel_impl(grad_output: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    print("Hello from hardsigmoid_backward_kernel_impl")
    ...
    if grad_output.shape != x.shape:
        raise ValueError(
            f"Shape mismatch: grad_output.shape={grad_output.shape} vs x.shape={x.shape}"
        )
    if grad_output.dtype != x.dtype:
        raise ValueError(
            f"Dtype mismatch: grad_output.dtype={grad_output.dtype} vs x.dtype={x.dtype}"
        )
