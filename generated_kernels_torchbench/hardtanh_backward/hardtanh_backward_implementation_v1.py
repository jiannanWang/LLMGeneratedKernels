...
def hardtanh_backward_kernel_impl(
    grad_output: torch.Tensor, inp: torch.Tensor, min_val: float, max_val: float
) -> torch.Tensor:
    
    print("Hello from hardtanh_backward_kernel_impl")
    ...
    if grad_output.shape != inp.shape:
        raise ValueError(
            f"Shape mismatch: grad_output.shape={grad_output.shape}, inp.shape={inp.shape}"
        )

    if grad_output.dtype != inp.dtype:
        raise ValueError(
            f"Dtype mismatch: grad_output.dtype={grad_output.dtype}, inp.dtype={inp.dtype}"
        )

    # Supported dtypes: float16, bfloat16, float32
    if grad_output.dtype not in (torch.float16, torch.bfloat16, torch.float32):
        raise TypeError(
            f"Unsupported dtype: {grad_output.dtype}. Supported: float16, bfloat16, float32"
        )
