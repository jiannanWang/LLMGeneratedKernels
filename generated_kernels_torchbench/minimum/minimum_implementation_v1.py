...
def _minimum_broadcast_kernel(
    ...
    for i in range(RANK):
        ...
        dim_sz = tl.where(
            dim_sz == 0, 1, dim_sz
        )  # guard (n_elements > 0 implies no zeros, but safe)
        idx_k = curr % dim_sz
        curr = curr // dim_sz
...
def minimum_kernel_impl(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    print("Hello from minimum_kernel_impl")
    ...
    rank = a_exp.dim()
    shape_t, a_strides_t, b_strides_t = _make_index_tensors(
        out_shape, a_exp.stride(), b_exp.stride(), a.device
    )
