...
def where_kernel_impl(cond: torch.Tensor, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    print("Hello from where_kernel_impl")
    ...
    sizes = torch.tensor(
        out_shape if len(out_shape) > 0 else [1], device=x.device, dtype=torch.int64
    )
    cond_strides = (
        torch.tensor(_aligned_strides(cond, out_shape), device=x.device, dtype=torch.int64)
        if len(out_shape) > 0
        else torch.tensor([0], device=x.device, dtype=torch.int64)
    )
    x_strides = (
        torch.tensor(_aligned_strides(x, out_shape), device=x.device, dtype=torch.int64)
        if len(out_shape) > 0
        else torch.tensor([0], device=x.device, dtype=torch.int64)
    )
    y_strides = (
        torch.tensor(_aligned_strides(y, out_shape), device=x.device, dtype=torch.int64)
        if len(out_shape) > 0
        else torch.tensor([0], device=x.device, dtype=torch.int64)
    )
