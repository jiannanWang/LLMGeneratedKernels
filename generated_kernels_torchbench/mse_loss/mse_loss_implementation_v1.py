...
def _prepare_meta(x: torch.Tensor, y: torch.Tensor, max_dims: int = 8):
    ...
    if len(sizes_list) > max_dims:
        raise ValueError(
            f"Input has {len(sizes_list)} dims, but only up to {max_dims} are supported."
        )
...
def mse_loss_kernel_impl(input: torch.Tensor, target: torch.Tensor, reduction: str = "mean"):
    print("Hello from mse_loss_kernel_impl")
    """
    ...
    if reduction == "none":
        ...
        out = torch.empty_strided(
            size=input.shape, stride=input.stride(), dtype=input.dtype, device=input.device
        )
