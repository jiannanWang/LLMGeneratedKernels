...
@triton.jit
def _hardswish_backward_kernel(go_ptr, x_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    """
...
def hardswish_backward_kernel_impl(grad_output: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    print("Hello from hardswish_backward_kernel_impl")
    ...
    if grad_output.shape != x.shape:
        raise ValueError(
            f"Shape mismatch: grad_output.shape={grad_output.shape}, x.shape={x.shape}"
        )
    ...
    if grad_output.dtype != x.dtype:
        raise ValueError(
            f"Dtype mismatch: grad_output.dtype={grad_output.dtype}, x.dtype={x.dtype}"
        )
