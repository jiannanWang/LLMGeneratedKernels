...
def _compute_base_offsets(
    shape: List[int], strides: List[int], keep_dims: List[int]
...
def var_mean_kernel_impl(
    ...
    *,
    ...
    if x.numel() == 0:
        ref_var, ref_mean = torch.ops.aten.var_mean.correction(
            x, dim, correction=correction, keepdim=keepdim
        )
    ...
    red_shapes, red_strides, NRED = _prepare_reduction_meta(
        shape, strides, red_dims, MAX_RED_DIMS, device=device
    )
