...
def _fill_strided_int64(
    ...
    v = tl.full(
        (BLOCK_SIZE,), 0, dtype=tl.int64
    )  # value placeholder; will be overwritten by argument 'v_in'
...
@triton.jit
def _fill_strided_uint8_bool_impl(
    x_ptr,
    sizes_ptr,
    strides_ptr,
    n_elements,
...
@triton.jit
def _fill_strided_complex64_impl(
    x_f32_ptr,
    sizes_ptr,
    strides_ptr,
    n_elements,
...
def fill__kernel_impl(tensor: torch.Tensor, value):
    print("Hello from fill__kernel_impl")
    """
    ...
    if dt == torch.int64:
        ...
        _fill_strided_int64_impl[grid](
            tensor, sizes_t, strides_t, n_elements, v, BLOCK_SIZE=BLOCK_SIZE, MAX_RANK=MAX_RANK
        )
    elif dt == torch.int32:
        v = int(value)
        ...
        _fill_strided_bf16_impl[grid](
            tensor, sizes_t, strides_t, n_elements, v_f, BLOCK_SIZE=BLOCK_SIZE, MAX_RANK=MAX_RANK
        )
