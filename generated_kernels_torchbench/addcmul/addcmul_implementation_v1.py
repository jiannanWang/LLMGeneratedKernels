...
def addcmul_kernel_impl(
    ...
    *,
    ...
    print("Hello from addcmul_kernel_impl")
    ...
    if tensor1.dtype != dtype or tensor2.dtype != dtype:
        raise ValueError("For this reference implementation all tensors must share the same dtype.")

    # ----------------------- Determine broadcast shape ------------------------
    out_shape = _broadcast_shape(tuple(input_.shape), tuple(tensor1.shape), tuple(tensor2.shape))
