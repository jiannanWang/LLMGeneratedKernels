...
def _log_softmax_backward_kernel(
    ...
    for offset in tl.range(0, COLS, BLOCK_SIZE):
        ...
        go_block = tl.load(grad_out_ptr + row_start + offs, mask=mask, other=tl.zeros((), DTYPE))
    ...
    for offset in tl.range(0, COLS, BLOCK_SIZE):
        ...
        go_block = tl.load(grad_out_ptr + row_start + offs, mask=mask, other=tl.zeros((), DTYPE))
        out_block = tl.load(out_ptr + row_start + offs, mask=mask, other=tl.zeros((), DTYPE))

        exp_out = tl.exp(out_block.to(tl.float32))  # e^{log_softmax} = softmax
        grad_block = go_block.to(tl.float32) - exp_out * row_sum

        tl.store(grad_in_ptr + row_start + offs, grad_block.to(DTYPE), mask=mask)


# ----------------------------------------------------------------------
# Public wrapper – this is what the unit-test imports and calls.
# ----------------------------------------------------------------------
def _log_softmax_backward_data_kernel_impl(
    grad_output: torch.Tensor, output: torch.Tensor, dim: int, dtype: torch.dtype
) -> torch.Tensor:
    print("Hello from _log_softmax_backward_data_kernel_impl")
    """
    ...
    assert grad_output.dtype in (torch.float16, torch.bfloat16), "Only FP16 / BF16 supported"
    assert grad_output.dtype == output.dtype == dtype, "Input dtypes mismatch"
    ...
    # ------------------------------------------------------------------
    if needs_inverse:
        grad_input = grad_input_flat.view(*grad_output_t.shape).permute(inverse_perm).contiguous()
    else:
