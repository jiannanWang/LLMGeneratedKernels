...
def sum_kernel_impl(x: torch.Tensor, dims, keepdim: bool, dtype: torch.dtype = None):
    print("Hello from sum_kernel_impl")
    """
    ...
    keep_dims, keep_shape, keep_strides, keep_cumprod = _build_keep_struct(
        in_shape, in_strides, reduce_set
    )
    outer_numel = _prod(keep_shape)  # number of output elements

    # Reduced dims structures and offsets
    red_shape, red_strides, red_cumprod, red_offsets = _build_reduce_struct(
        in_shape, in_strides, dims_list
    )
    ...
    keep_shape_pad = torch.tensor(
        pad_list(keep_shape, 1, MAX_DIMS), device=x.device, dtype=torch.int64
    )
    keep_cumprod_pad = torch.tensor(
        pad_list(keep_cumprod, 1, MAX_DIMS), device=x.device, dtype=torch.int64
    )
    keep_strides_pad = torch.tensor(
        pad_list(keep_strides, 0, MAX_DIMS), device=x.device, dtype=torch.int64
    )

    # Reduction offsets buffer (can be zero-sized)
    red_offsets_t = torch.tensor(
        red_offsets if len(red_offsets) > 0 else [0], device=x.device, dtype=torch.int64
    )
    if red_total == 0:
